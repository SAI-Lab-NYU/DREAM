...
base_dataset_path = "/home/asperger/datasets/llava-v1.5-instruct/download/llava-v1.5-instruct"
...
def load_image(base_path, image_path):
    try:
        ...
        print(f"无法打开图像 {image_path}，错误信息：{e}")
        img = None  # 确保 img 被赋值
    return img
...
def prune_image_tokens(
    ...
    for b in range(B):
        ...
        new_target_attention.append(attentions[b][keep_mask])
...
ds = load_dataset("mrm8488/llava_v1_5_mix665k", split="train[:1000]")
batch_size = 1
...
def ge(data):
    ...
    new_input_ids, new_embeds, new_target = prune_image_tokens(inputs.input_ids, outs_big.hidden_states[0], outs_big.hidden_states[-1])
    ...
    td[f"inputs_embeds"] = new_embeds.to(torch.bfloat16).cpu()
    # td[f"hidden_state_layer8"] = outs_big.hidden_states[8].to(torch.bfloat16).cpu()
    # td[f"hidden_state_layer16"] = outs_big.hidden_states[16].to(torch.bfloat16).cpu()
    # td[f"hidden_state_layer24"] = outs_big.hidden_states[24].to(torch.bfloat16).cpu()
    ...
    td[f"target"] = new_target.to(torch.bfloat16).cpu()
